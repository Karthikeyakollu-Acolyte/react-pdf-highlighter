...
> {
  ...
  componentDidUpdate(prevProps: Props<T_HT>) {
    if (prevProps.pdfDocument !== this.props.pdfDocument) {
      // If the pdfDocument has changed, re-initialize the viewer
      this.init();
      return;
    }

    if (prevProps.highlights !== this.props.highlights) {
      // If highlights have changed, re-render the highlight layers
      this.renderHighlightLayers();
    }

    if (prevProps.page !== this.props.page && this.viewer) {
      // If page number has changed, update the current page in the viewer
      this.viewer.currentPageNumber = this.props.page;
    }
  }


  async init() {
    const { pdfDocument, pdfViewerOptions, page } = this.props;
    ...
    this.viewer.eventBus.on("pagesinit", () => {
      this.viewer.currentPageNumber = page;
    });

    console.log(this.viewer)
  
    this.viewer.container.style.scrollBehavior = "smooth";
